...
topic = "robot/aafa75a2-fd7a-4302-9fd9-ea8402983f0f-0hpwuf5-viewer"
...
piper_dof_props['stiffness'] = 1e5
piper_dof_props['damping'] = 100.0        #200.0
...
def update_piper(t):
    ...
    for i in range(num_envs):
        ...
        if joints_copy:
            grip_input = joints_copy[6]  # e.g., 11
            grip_input_clipped = np.clip(grip_input, -1, 89)
            grip_norm = (grip_input_clipped + 1) / 90.0  # maps [-1, 89] → [0.0, 1.0]
            
            # Map to DOF range
            grip_pos_6 = 0.0 + grip_norm * 0.04       # DOF 6: [0.0, 0.04]
            grip_pos_7 = 0.0 - grip_norm * 0.04       # DOF 7: [0.0, -0.04]

            piper_dof_states['pos'][6] = grip_pos_6
            piper_dof_states['pos'][7] = grip_pos_7
            
            for j in range(6):
                # piper_dof_states['vel'][j] = 0.0
                joint_angle = joints_copy[j] * math.pi / 180.0
                # joint_angle_clamped = np.clip(joint_angle, piper_lower_limits[j], piper_upper_limits[j])
                piper_dof_states['pos'][j] = joint_angle
                # print(f"joint[{j+1}]: {(states_bf[j] - piper_dof_states['pos'][j])*180/math.pi:.5f}")
            # diff = states_bf - piper_dof_states['pos']
            # print("diff:", np.round(diff, 6))
            ...
            # gym.set_actor_dof_position_targets(envs[i], piper_handles[i], piper_dof_states)
